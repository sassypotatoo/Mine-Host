#!/usr/bin/env bash
#
# Beast Mode v3 - Autonomous Workflow Handler
#
# Handles the complete autonomous workflow when Beast Mode is ON
# Turn-based state machine: IMPLEMENT -> ADVANCING -> VERIFYING -> (VERIFIED|FAILED_RETRY)
#

set -euo pipefail

# Use absolute path to avoid any directory confusion
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "${PROJECT_ROOT}/.claude/scripts/beastmode_utils.sh"

STATE_FILE="${PROJECT_ROOT}/.claude/beastmode_state.json"
MAX_ATTEMPTS_PER_OBJECTIVE=5
MAX_TOTAL_ITERATIONS=15

# Function to classify user intent semantically
# Returns: QUESTION, WORK_REQUEST, or CONTROL_REQUEST
classify_intent() {
    local message="$1"

    # Simple heuristic-based classification for now
    # In a full implementation, this would use more sophisticated NLP

    # Control request patterns
    if [[ "$message" =~ ^/(pause|resume|cancel|stop|toggle|task-change) ]]; then
        echo "CONTROL_REQUEST"
        return
    fi

    # Work request patterns - action-oriented language
    if [[ "$message" =~ ^/(fix|implement|add|create|remove|delete|update|change|modify) ]] ||
       [[ "$message" =~ (|can you|could you)(.*)?(fix|implement|add|create|remove|delete|update|change|modify) ]] ||
       [[ "$message" =~ (need|want|require)(.*)?(to|for)(.*)?(fix|implement|add|create|remove|delete|update|change|modify) ]] ||
       [[ "$message" =~ ^[A-Z][a-z]+(.+)(fix|implement|add|create|remove|delete|update|change|modify) ]]; then
        echo "WORK_REQUEST"
        return
    fi

    # Default to question/discussion
    echo "QUESTION"
}

# Function to decompose work request into objectives
decompose_work_request() {
    local work_request="$1"

    # Simple decomposition - in reality, this would be more sophisticated
    # For now, treat the entire request as one objective unless it clearly contains multiple parts

    # Check for obvious multi-part requests
    if [[ "$work_request" =~ (and|then|also|also |also ) ]] &&
       [[ "$work_request" =~ (fix|implement|add|create|remove|delete|update|change|modify).*(fix|implement|add|create|remove|delete|update|change|modify) ]]; then
        # Split by common conjunctions for multi-objective tasks
        echo "$work_request" | python3 - <<'PY'
import sys, re
request = sys.stdin.read().strip()
# Split by common conjunctions that indicate separate objectives
parts = re.split(r'\s+(and|then|also|also |also )\s+', request, flags=re.IGNORECASE)
# Filter out the conjunctions themselves
objectives = [parts[i] for i in range(0, len(parts), 2) if i < len(parts)]
for obj in objectives:
    if obj.strip():
        print(obj.strip())
PY
    else
        # Single objective
        echo "$work_request"
    fi
}

# Function to implement an objective (provides guidance to Claude Code)
implement_objective() {
    local objective_description="$1"
    local objective_id="$2"

    echo "Implementing objective: $objective_description"
    echo "Objective ID: $objective_id"

    # For Phase 2, we attempt to implement common objective types directly
    # and provide guidance to Claude Code for more complex tasks

    # Check if this is a simple file modification we can handle
    if [[ "$objective_description" =~ ^(Fix|fix|Update|update)[[:space:]](.+)\.(md|txt|json|xml|yaml|yml)[[:space:]]*$ ]]; then
        # Simple file fix - attempt to locate and fix the file
        local target_file
        target_file=$(echo "$objective_description" | sed -E 's/^(Fix|fix|Update|update)[[:space:]](.+)\.(md|txt|json|xml|yaml|yml)[[:space:]]*$/\2.\3/')

        if [ -f "$PROJECT_ROOT/$target_file" ]; then
            echo "Attempting to fix $target_file based on objective: $objective_description"
            # For now, we'll signal that Claude Code should handle this
            # In a more advanced implementation, we could attempt simple fixes
            echo "GUIDANCE_NEEDED: implement the following objective: $objective_description"
            return 0
        fi
    fi

    # For all other objectives, provide clear guidance to Claude Code
    echo "GUIDANCE_PROVIDED: Objective requires Claude Code implementation"
    echo "Objective Details: $objective_description"
    echo "implement this objective in the codebase."

    return 0
}

# Function to commit and push changes
# Outputs progress messages to stderr so stdout carries ONLY the commit SHA
commit_and_push() {
    local commit_message="$1"

    echo "Committing and pushing changes." >&2
    echo "Commit message: $commit_message" >&2

    # Commit changes using git
    cd "$PROJECT_ROOT"
    git add -u
    if [ $? -ne 0 ]; then
        echo "Error: git add -u failed" >&2
        return 1
    fi
    git add . 2>/dev/null || true # Add new files
    git commit -m "$commit_message"
    if [ $? -ne 0 ]; then
        echo "Error: git commit failed" >&2
        return 1
    fi

    # Get the commit SHA
    local commit_sha
    commit_sha=$(git rev-parse HEAD)
    if [ -z "$commit_sha" ]; then
        echo "Error: failed to get commit SHA" >&2
        return 1
    fi
    echo "Committed with SHA: $commit_sha" >&2

    # Push using ./tools/push-gated.sh
    echo "Pushing using ./tools/push-gated.sh" >&2
    if ! ./tools/push-gated.sh >&2; then
        echo "Error: ./tools/push-gated.sh failed" >&2
        return 1
    fi

    # Return the actual commit SHA
    echo "$commit_sha"
}

# Function to verify implementation via CI
verify_via_ci() {
    local commit_sha="$1"

    echo "Verifying via CI for commit: $commit_sha"

    # Use ./tools/ci-watch.sh to monitor the CI run for this commit
    # This will update the state via ./tools/ci-state-update.sh
    echo "Monitoring CI for commit $commit_sha using ./tools/ci-watch.sh"
    if ! ./tools/ci-watch.sh --sha "$commit_sha" --update-state; then
        echo "ERROR: Failed to monitor CI for commit $commit_sha" >&2
        return 1
    fi

    # Check if CI passed by reading the updated state
    local state_json
    state_json=$(read_state)
    local ci_status
    ci_status=$(echo "$state_json" | python3 -c "
import sys, json
data = json.load(sys.stdin)
commit_sha = sys.argv[1]
# Find the objective with this commit SHA and get its CI status
for obj in data['objectives']:
    if obj.get('lastCommit') == commit_sha:
        print(obj.get('ciStatus', ''))
        break
    " "$commit_sha")

    if [ "$ci_status" = "PASS" ]; then
        echo "CI_RESULT: PASS"
        return 0
    else
        echo "CI_RESULT: FAIL"
        return 1
    fi
}

# Function to handle CI failure
handle_ci_failure() {
    local commit_sha="$1"
    local objective_description="$2"
    local objective_id="$3"
    local attempt_num="$4"

    echo "Handling CI failure for attempt $attempt_num"
    echo "Commit SHA: $commit_sha"
    echo "Objective: $objective_description"

    # In a real implementation:
    # 1. Retrieve actual CI logs
    # 2. Classify the failure (compile error, test failure, etc.)
    # 3. Diagnose root cause
    # 4. Fix the appropriate problem

    # Retrieve actual CI logs using gh
    echo "Retrieving CI logs for commit $commit_sha"
    local ci_logs
    ci_logs=$(./tools/ci-watch.sh --sha "$commit_sha" --get-logs 2>/dev/null || echo "Failed to retrieve CI logs")

    # Signal that Claude Code should analyze the CI failure
    local temp_ci_file="${PROJECT_ROOT}/.claude/ci_analysis_${objective_id}_${attempt_num}.txt"
    echo "Commit SHA: $commit_sha" > "$temp_ci_file"
    echo "Objective: $objective_description" >> "$temp_ci_file"
    echo "Attempt: $attempt_num" >> "$temp_ci_file"
    echo "" >> "$temp_ci_file"
    echo "CI Logs:" >> "$temp_ci_file"
    echo "$ci_logs" >> "$temp_ci_file"
    echo "" >> "$temp_ci_file"
    echo "Analysis request: Identify root cause and suggest smallest appropriate fix following MineHost's non-negotiables." >> "$temp_ci_file"

    # Signal that Claude Code should analyze this failure
    # Claude Code will observe the state and temp file, then provide analysis
    echo "CLAUDE_CODE_ANALYZE_FAILURE: $objective_id:$attempt_num"

    # For Phase 2, we provide a basic analysis based on common CI failure patterns
    # In a full implementation, Claude Code would analyze the actual logs and provide a fix
    local fix_analysis="Analyze CI logs above to identify root cause and apply smallest appropriate fix"

    # Return the fix description
    echo "$fix_analysis"
}

# Function to check if local verification is available and useful
should_use_local_verification() {
    # Check if we have genuine local verification capabilities
    if is_local_verification_available; then
        echo "Local verification is available"
        # Additional checks could go here
        return 0
    else
        echo "Local verification not available - relying on CI"
        return 1
    fi
}

# Function to handle the --intake subcommand
handle_intake() {
    local work_request="$1"

    echo "Starting Beast Mode autonomous workflow"
    echo "Work request: $work_request"

    # Reset state for new work request
    local state_json
    state_json=$(read_state)
    local new_state
    new_state=$(echo "$state_json" | python3 - "$work_request" "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
# Only reset objectives if this is a new work request (different from current task)
if data.get('currentTask', '') != sys.argv[1]:
    data['objectives'] = []
    data['currentTask'] = sys.argv[1]
    data['currentObjectiveId'] = None
    data['gitBranch'] = ''
    data['lastCiRunUrl'] = ''
    data['knownIssues'] = []
    data['blockers'] = []
    data['timestamp'] = sys.argv[2]
    data['phase'] = 'IMPLEMENT'
print(json.dumps(data))
PY
    )
    write_state "$new_state"

    # Decompose work request into objectives
    echo "Decomposing work request into objectives."
    local objectives
    objectives=$(decompose_work_request "$work_request")

    # Check if objectives for this work request already exist
    local existing_objectives_json
    existing_objectives_json=$(read_state | python3 -c "
import sys, json
data = json.load(sys.stdin)
existing = [obj['description'] for obj in data.get('objectives', []) if obj.get('status') in ('PENDING', 'IN_PROGRESS')]
print(','.join(existing))
    ")

    # Add objectives to state
    local objective_count=0
    while IFS= read -r objective_line; do
        if [ -n "$objective_line" ]; then
            # Check if this objective already exists
            if echo "$existing_objectives_json" | grep -q "$objective_line"; then
                echo "Objective already exists, reusing: $objective_line"
                objective_count=$((objective_count + 1))
                continue
            fi
            local obj_id
            obj_id=$(add_objective "$objective_line")
            echo "Added objective $obj_id: $objective_line"
            objective_count=$((objective_count + 1))
        fi
    done < <(echo "$objectives")

    echo "Created $objective_count objectives"

    # Set the current objective to the first PENDING one if we have any
    if [ $objective_count -gt 0 ]; then
        local first_obj_id
        first_obj_id=$(get_next_pending_objective_id)
        set_current_objective "$first_obj_id"
        echo "Set current objective to: $first_obj_id"
    else
        # No objectives, set to IDLE
        local state_json
        state_json=$(read_state)
        local new_state
        new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IDLE'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
        )
        write_state "$new_state"
        echo "No objectives found, setting phase to IDLE"
    fi
}

# Function to handle the --advance subcommand
handle_advance() {
    # Read current state
    local state_json
    state_json=$(read_state)
    local phase
    phase=$(echo "$state_json" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('phase', 'IDLE'))")
    local current_task
    current_task=$(echo "$state_json" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('currentTask', ''))")
    local current_objective_id
    current_objective_id=$(echo "$state_json" | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('currentObjectiveId', ''))")

    echo "Current phase: $phase"
    echo "Current task: $current_task"
    echo "Current objective ID: $current_objective_id"

    case "$phase" in
        IMPLEMENT)
            # If no current objective or it's not PENDING, get the next PENDING objective
            if [ -z "$current_objective_id" ]; then
                local next_obj_id
                next_obj_id=$(get_next_pending_objective_id)
                if [ -z "$next_obj_id" ]; then
                    echo "No pending objectives found, setting phase to IDLE"
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IDLE'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                    return 0
                fi
                set_current_objective "$next_obj_id"
                current_objective_id="$next_obj_id"
                echo "Set current objective to: $current_objective_id"
            fi

            # Get the current objective details
            local obj_desc
            obj_desc=$(get_objective_field "$current_objective_id" "description")
            local obj_attempts
            obj_attempts=$(get_objective_field "$current_objective_id" "attempts")
            if [ -z "$obj_attempts" ]; then
                obj_attempts=0
            fi

            echo "Implementing objective: $obj_desc"
            echo "Attempt: $((obj_attempts + 1))"

            # Provide guidance to Claude Code
            implement_objective "$obj_desc" "$current_objective_id"

            # Now, we assume the user will implement and then trigger the next turn via a new prompt
            # In this turn, we just set up for implementation and exit.
            # The actual implementation is done by Claude Code in response to the GUIDANCE_NEEDED.
            # We will commit and push in the next turn? Actually, we want to commit after implementation.
            # But we don't know when the user is done. So we will wait for the user to signal completion
            # by sending a prompt that indicates they are ready to commit.

            # For now, we will just exit and wait for the next user prompt.
            # The next prompt will trigger --advance again, and we will still be in IMPLEMENT phase.
            # We need a way to know when the user has implemented.

            # Instead, we will change the approach: in the IMPLEMENT phase, we will attempt to
            # implement the objective ourselves (if possible) or provide guidance and then
            # wait for the user to confirm implementation by sending a specific signal.

            # However, to keep it simple for now, we will assume that the user will implement
            # and then send a prompt like "done" or "implemented". We will then check for that
            # in the hook? But we cannot change the hook arbitrarily.

            # Let's reconsider: the hook calls the workflow script on every user prompt.
            # We want:
            #   - User sends work request -> hook calls --intake -> sets phase to IMPLEMENT
            #   - Hook injects GUIDANCE_NEEDED and returns the prompt to Claude Code
            #   - Claude Code sees the GUIDANCE_NEEDED and implements the objective
            #   - Claude Code then sends a prompt (e.g., "I have implemented the objective")
            #   - Hook calls --advance again, and we are still in IMPLEMENT phase
            #   - Now we can commit and push the changes (assuming Claude Code has made them)
            #   - Then we set phase to ADVANCING and spawn CI watch

            # So we need to detect when the user has implemented. We can do that by checking
            # if there are any unstaged changes? Or we can just assume that if we are in
            # IMPLEMENT phase and the user sends a prompt, they have implemented.

            # We'll go with that: in IMPLEMENT phase, we treat any user prompt as a signal
            # that implementation is done, and we commit and push.

            # Commit and push changes
            local commit_message
            commit_message="Beast Mode: Implement objective $current_objective_id - $obj_desc"
            local commit_sha
            if ! commit_sha=$(commit_and_push "$commit_message"); then
                echo "ERROR: Failed to commit and push changes for objective $current_objective_id"
                # We stay in IMPLEMENT phase so the user can try again
                return 1
            fi

            # Update state with commit info
            state_json=$(read_state)
            new_state=$(echo "$state_json" | python3 -c "
import sys, json
data = json.load(sys.stdin)
obj_id = sys.argv[1]
commit_sha = sys.argv[2]
timestamp = sys.argv[3]
for obj in data['objectives']:
    if obj['id'] == obj_id:
        obj['lastCommit'] = commit_sha
        break
data['timestamp'] = timestamp
print(json.dumps(data))
            " "$current_objective_id" "$commit_sha" "$(get_timestamp)")
            write_state "$new_state"

            # Spawn CI watch in background
            echo "Starting CI watch for commit $commit_sha"
            ./tools/ci-watch.sh --sha "$commit_sha" --update-state >/dev/null 2>&1 &
            echo "CI watch started in background"

            # Set phase to ADVANCING
            state_json=$(read_state)
            new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'ADVANCING'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
            )
            write_state "$new_state"
            echo "Phase set to ADVANCING"
            ;;
        ADVANCING)
            # We assume the background CI watch has updated the state via ci-state-update.sh
            # Now we move to VERIFYING to check the result
            state_json=$(read_state)
            new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'VERIFYING'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
            )
            write_state "$new_state"
            echo "Phase set to VERIFYING"
            ;;
        VERIFYING)
            # Check the CI status of the current objective
            if [ -z "$current_objective_id" ]; then
                echo "No current objective, setting phase to IDLE"
                state_json=$(read_state)
                new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IDLE'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                )
                write_state "$new_state"
                return 0
            fi

            local state_json
            state_json=$(read_state)
            local ci_status
            ci_status=$(echo "$state_json" | python3 -c "
import sys, json
data = json.load(sys.stdin)
obj_id = sys.argv[1]
for obj in data['objectives']:
    if obj['id'] == obj_id:
        print(obj.get('ciStatus', ''))
        break
            " "$current_objective_id")

            if [ "$ci_status" = "PASS" ]; then
                echo "CI PASSED for objective $current_objective_id"

                # Mark objective as verified
                state_json=$(read_state)
                new_state=$(echo "$state_json" | python3 - "$current_objective_id" "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
obj_id = sys.argv[1]
timestamp = sys.argv[2]
for obj in data['objectives']:
    if obj['id'] == obj_id:
        obj['status'] = 'VERIFIED'
        obj['verificationNotes'] = 'CI verification passed'
        break
data['timestamp'] = timestamp
print(json.dumps(data))
PY
                )
                write_state "$new_state"

                # Move to next objective
                    local next_obj_id
                    next_obj_id=$(get_next_pending_objective_id)
                    if [ -z "$next_obj_id" ]; then
                    echo "All objectives verified, setting phase to IDLE"
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IDLE'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                else
                    set_current_objective "$next_obj_id"
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IMPLEMENT'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                    echo "Phase set to IMPLEMENT for next objective"
                fi
            else
                echo "CI FAILED for objective $current_objective_id"

                # Handle failure
                local obj_desc
                obj_desc=$(get_objective_field "$current_objective_id" "description")
                local obj_attempts
                obj_attempts=$(get_objective_field "$current_objective_id" "attempts")
                if [ -z "$obj_attempts" ]; then
                    obj_attempts=0
                fi
                local attempt_num=$((obj_attempts + 1))

                local fix_description
                fix_description=$(handle_ci_failure "$(get_objective_field "$current_objective_id" "lastCommit")" "$obj_desc" "$current_objective_id" "$attempt_num")

                # Update state with failure info
                state_json=$(read_state)
                new_state=$(echo "$state_json" | python3 -c "
import sys, json
data = json.load(sys.stdin)
obj_id = sys.argv[1]
ci_status = sys.argv[2]
fix_desc = sys.argv[3]
timestamp = sys.argv[4]
for obj in data['objectives']:
    if obj['id'] == obj_id:
        obj['ciStatus'] = ci_status
        obj['verificationNotes'] = 'CI verification failed: ' + fix_desc
        break
data['timestamp'] = timestamp
print(json.dumps(data))
                " "$current_objective_id" "FAIL" "$fix_description" "$(get_timestamp)"
                write_state "$new_state"

                # Check if we have exceeded retry budget
                if [ $attempt_num -ge $MAX_ATTEMPTS_PER_OBJECTIVE ]; then
                    echo "ERROR: Objective $current_objective_id exceeded retry budget ($MAX_ATTEMPTS_PER_OBJECTIVE)"
                    # Mark as FAILED_EXCEEDED
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 -c "
import sys, json
data = json.load(sys.stdin)
obj_id = sys.argv[1]
timestamp = sys.argv[2]
for obj in data['objectives']:
    if obj['id'] == obj_id:
        obj['status'] = 'FAILED_EXCEEDED'
        obj['verificationNotes'] = 'Exceeded retry budget ($MAX_ATTEMPTS_PER_OBJECTIVE attempts)'
        break
data['timestamp'] = timestamp
print(json.dumps(data))
                    ) "$current_objective_id" "$(get_timestamp)"
                    write_state "$new_state"

                    # Move to next objective
                    local next_obj_id
                    next_obj_id=$(get_next_pending_objective_id)
                    if [ -z "$next_obj_id" ]; then
                        echo "No more objectives, setting phase to IDLE"
                        state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IDLE'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                else
                    set_current_objective "$next_obj_id"
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IMPLEMENT'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                    echo "Phase set to IMPLEMENT for next objective"
                else
                    # Retry: set phase back to IMPLEMENT
                    state_json=$(read_state)
                    new_state=$(echo "$state_json" | python3 - "$(get_timestamp)" <<'PY'
import sys, json
data = json.load(sys.stdin)
data['phase'] = 'IMPLEMENT'
data['timestamp'] = sys.argv[1]
print(json.dumps(data))
PY
                    )
                    write_state "$new_state"
                fi
            ;;
        IDLE|*)
            echo "No action needed for phase: $phase"
            ;;
    esac
}

# Main script entry point
main() {
    # Check if we were called with a subcommand
    if [ $# -eq 0 ]; then
        echo "Beast Mode Workflow Handler"
        echo "Usage: $0 {--intake \"work request\"|--advance}"
        exit 1
    fi

    local subcommand="$1"
    shift

    case "$subcommand" in
        --intake)
            if [ $# -eq 0 ]; then
                echo "Error: --intake requires a work request argument"
                exit 1
            fi
            handle_intake "$*"
            ;;
        --advance)
            handle_advance
            ;;
        *)
            echo "Error: unknown subcommand '$subcommand'"
            echo "Usage: $0 {--intake \"work request\"|--advance}"
            exit 1
            ;;
    esac
}

# If script is executed directly, run main
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    main "$@"
fi